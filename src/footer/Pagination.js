  ...
  return (
    <div className="pagination container justify-content-center mt-3"><nav aria-label="Page navigation">
      ...
      <ul className="pagination justify-content-center">
        ...
        {arrayCount(Math.floor(props.countPagination - count / 2), props.countPagination < count ?Math.floor(LIMIT) < count?Math.floor(LIMIT): count: (props.countPagination - 0 + count / 2)).map((x, i) => <li className={"page"}><span onClick={(e) => { props.setCountPagination(e.target.innerHTML); }} className={props.countPagination - 1 === i ? "page-link e-aktive" : "page-link no-aktive"}  >{(i + 1)}</span></li>)}
